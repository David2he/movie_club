...
export default class List_movie extends React.Component {
    constructor(props) {
      ...
      this.props = props
      this.state = {
        items: props.slice.items.filter((movie, index) => index < props.slice.primary.max_display),
    ...
    MenuLink = ({ menuLink }) => (
        <View style={styles.movie} className="movie">
          <Image alt={menuLink.poster} src={menuLink.poster} style={styles.poster}></Image>
